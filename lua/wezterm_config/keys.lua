...
local function build_target_heights(total_height, count)
  local heights = {}
  local base = math.floor(total_height / count)
  local remainder = total_height % count
...
local function equalize_rows(window, act, rows)
  ...
  local target_heights = build_target_heights(total_height, #rows)
  local current_prefix = 0
  local target_prefix = 0
