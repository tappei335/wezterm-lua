...
local function build_target_sizes(total_size, count)
  local sizes = {}
  local base = math.floor(total_size / count)
  local remainder = total_size % count
...
local function equalize_columns(window, act, columns)
  ...
  local target_widths = build_target_sizes(total_width, #columns)
...
local function equalize_rows(window, act, rows)
  ...
  local target_heights = build_target_sizes(total_height, #rows)
  local current_prefix = 0
  local target_prefix = 0
