...
どちらのスクリプトも `.wezterm.lua` と `lua/` を WezTerm の config directory にコピーします。
既定のコピー先は Linux では `$XDG_CONFIG_HOME/wezterm` または `~/.config/wezterm`、Windows では `%USERPROFILE%\.config\wezterm` です。
必要なら引数で別のコピー先を指定できます。
