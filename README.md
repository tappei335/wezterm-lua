...
- `lua/wezterm_config/ssh.lua`
  - `~/.ssh/config` の Host を SSH domain として登録し、SSH host selector と launch menu に追加します。
...
より tmux に近い remote session の永続化が必要な場合は、WezTerm が自動生成する `SSHMUX:*` domain を使えます。ただし remote 側にも互換バージョンの WezTerm が必要です。
